...
lag_cov <- function(data = NULL,
    ...
    if ((length(unique(data[[time]])) != nrow(data))) {
      stop("Duplicated time units detected, check option 'group'")
    }
    data <- data[order(data[[time]]), ]
    data <- data[, c(time, var)]    
    for (v in var) {
      for (i in lag[1]:lag[2]) {
        data[[paste0(v, "_lag_", i)]] <- c(rep(NA, i), data[[v]][1:(length(data[[v]]) - i)])
      }
    }
    out <- data
  } else if (!is.null(group) && group %in% names(data)) {
    data <- data[order(data[[group]], data[[time]]), ]
    groups <- split(data[, c(group, time, var)], data[[group]])
    lagged_data <- NULL
    for (v in var) {
      lagged_data[[v]] <- lapply(X = groups, FUN = function(x) {
        for (i in lag[1]:lag[2]) {
          x[[paste0(v, "_lag_", i)]] <- c(
            rep(NA, i),
            x[[v]]
          )[1:length(x[[v]])]
        }
    ...
    within_var <- lapply(lagged_data, FUN = function(x) {
      do.call(rbind, x)
    })
    out <- Reduce(
      function(x, y) merge(x, y, by = c(group, time, var), all = TRUE),
      within_var
    )
  }
  out <- out %>%
  ...
  if (merge == TRUE) {
    out <- data %>% dplyr::left_join(out, by = c(
      {group}, {time}))
  } else {
   out <- out
  }
